...
<div class="container mt-4 text-light">
    ...
    <div class="mb-3 d-flex gap-2 flex-wrap">
        ...
        <button class="btn btn-warning" onclick="loadLog('steam')">⚙️ Log de Steam</button>
        <button class="btn btn-danger ms-auto" onclick="clearLog()" id="clearBtn" disabled>🧹 Limpiar Log</button>
        <button class="btn btn-outline-light" onclick="toggleAutoscroll()" id="autoBtn">🔽 Autoscroll: ON</button>
    </div>

    <pre id="logContent" style="background:#000;color:#0f0;padding:15px;height:420px;overflow-y:auto;border-radius:10px;">
...
function loadLog(type){
  ...
  function fetchLog(){
    fetch('pages/logs_api.php?action=view&file=' + encodeURIComponent(type))
      .then(r=>r.json())
      .then(j=>{
...
function clearLog(){
  ...
  fetch('pages/logs_api.php?action=clear', {
      method:'POST',
